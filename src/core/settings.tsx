  ...
  return (
    <Fragment>
      ...
      <Modal title={t('Settings')} show={showSettings} onClose={toggleSettings} class="max-w-lg">
        ...
        <p class={styles.subtitle}>{t('LocalDownloadSettings')}</p>
        <div class={cx(styles.block, 'flex-col')}>
          <label class={styles.item}>
            <span class="label-text">{t('DownloadPathUrl')}</span>
            <input
              type="text"
              class="input input-bordered input-xs w-48"
              value={options.get('downloadPathUrl')}
              onChange={(e) => {
                options.set('downloadPathUrl', (e.target as HTMLInputElement)?.value);
              }}
            />
          </label>
        </div>
