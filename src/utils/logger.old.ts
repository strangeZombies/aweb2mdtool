...
class Logger {
  ...
  private t: (key: TranslationKey) => string; // 翻译函数

  constructor() {
    // 使用 useTranslation 获取翻译函数
   ...
   * 记录信息消息。
   */
  public info(line: TranslationKey, ...args: LogExtraArgs): void {
    const translatedLine = this.t(line); // 获取翻译后的内容
    this.logToConsole('info', translatedLine, ...args);
    this.writeBuffer({ type: 'info', line: translatedLine, index: this.index++ });
  }

  /**
   * 记录警告消息。
   */
  public warn(line: TranslationKey, ...args: LogExtraArgs): void {
    const translatedLine = this.t(line); // 获取翻译后的内容
    this.logToConsole('warn', translatedLine, ...args);
    this.writeBuffer({ type: 'warn', line: translatedLine, index: this.index++ });
  }

  /**
   * 记录错误消息。
   */
  public error(line: TranslationKey, ...args: LogExtraArgs): void {
    const translatedLine = this.t(line); // 获取翻译后的内容
    this.logToConsole('error', translatedLine, ...args);
    this.writeBuffer({ type: 'error', line: translatedLine, index: this.index++ });
  }
